            ...
            <div class="col-xl-8 col-lg-7 col-md-12 p-0">
                <div class="col-lg-12 col-12 order-2 order-lg-1">
                    <div class="image-block">
                        @if (!empty($program->thumbnail_img))
                            @if(file_exists('uploads/medias/'.$program->thumbnail_img))
                                <img src="{{asset('uploads/medias/'.$program->thumbnail_img)}}" class="img-fluid">
                            @else
                ...
                <div class="col-lg-12 col-12 mb-5 mb-lg-0 order-3 order-lg-3">
                    <div class="section-title h-100 d-flex flex-column justify-content-center">
                        ...
                        <p>{{date('D, d M, Y',strtotime($program->date))}}</p>
                        <p class="text_gray_ptext">{!! $program->description !!}</p>
                    </div>
                </div>
                @if (!empty($program->image))
                    @php
                        $photos = json_decode($program->image);
                    @endphp                    
                    <div class="col-lg-12 col-12 mb-5">
                        <div class="section-title h-100 d-flex flex-column justify-content-center">
                            <h2 class="pt-4 font-weight-bold">Related Images
                            </h2>
                                ...
                                <div id="lightgallery" class="row p-0">
                                    @foreach ($photos as $photo)
                                        {{-- @if (!empty($photo->photos)) --}}
                                            @if(file_exists('uploads/medias/'.$photo))
                                                <div class="col-xl-3 col-md-6 col-12" data-src="{{asset('uploads/medias/'.$photo)}}">
                                                    <a href="{{asset('uploads/medias/'.$photo)}}" target="_blank">
                                                        <img src="{{asset('uploads/medias/'.$photo)}}" class="img-fluid img-responsive">
                                                    </a>
                                                </div>
                                            @else
